...
import { runNBAResearch, formatNBAReportForPipeline, NBAAnalysisOptions } from './nbaAnalysis';
import { runNBAQuantModel, convertNBAQuantToAnalysisResult } from './nbaQuantModel';
...
// ═══════════════════════════════════════════════════════════════
// NBA ANALYSIS - INDEPENDENT PIPELINE
// ═══════════════════════════════════════════════════════════════

/**
 * NBA Analysis Pipeline - INDEPENDIENTE del pipeline de fútbol
 * 
 * Este pipeline es SEPARADO y NO modifica los steps existentes:
 * - Step 1: The Odds API (basketball_nba)
 * - Step NBA: Perplexity Sonar Pro (investigación NBA especializada)
 * - Step 3: Claude Sonnet 4.6 (análisis cuantitativo)
 * - Step 4: Grok 4.1 Fast (validación)
 */
export async function analyzeNBA(
  options: NBAAnalysisOptions & { onProgress?: (state: AnalysisState) => void }
): Promise<AnalysisResult> {
  const { matchName, date, onProgress } = options;

  // Initialize state with 4 steps for NBA
  const initialState: AnalysisState = {
    currentStep: 0,
    steps: [
      { step: 0, status: 'pending', icon: '🏀', message: 'Obteniendo cuotas NBA...', progress: 20 },
      { step: 1, status: 'pending', icon: '📊', message: 'Investigación NBA (Sonar Pro)...', progress: 45 },
      { step: 2, status: 'pending', icon: '🤖', message: 'Claude Sonnet analizando valor...', progress: 70 },
      { step: 3, status: 'pending', icon: '✅', message: 'Grok validando análisis...', progress: 90 }
    ],
    result: null,
    error: null
  };
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 0: THE ODDS API (NBA)
  // ═══════════════════════════════════════════════════════════════

  emitProgress(0, 'running');
  
  const { oddsPayload, warning: oddsWarning } = await fetchOddsFromAPI(matchName, 'basketball');
  
  if (oddsWarning) {
    console.log(`⚠️ Step 0 (Odds) warning: ${oddsWarning}`);
  }
  
  emitProgress(0, 'completed', oddsWarning);

  // ═══════════════════════════════════════════════════════════════
  // STEP 1: NBA RESEARCH (SONAR PRO)
  // ═══════════════════════════════════════════════════════════════

  emitProgress(1, 'running');
  
  const nbaResearch = await runNBAResearch({ matchName, date });
  const researchContext = formatNBAReportForPipeline(nbaResearch);
  
  const dataQuality: 'alta' | 'media' | 'baja' = nbaResearch.success ? 
    (nbaResearch.sources.length >= 3 ? 'alta' : 'media') : 'baja';
  
  if (!nbaResearch.success) {
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 2: NBA QUANT MODEL (CLAUDE SONNET)
  // ═══════════════════════════════════════════════════════════════

  emitProgress(2, 'running');
  
  // Usar el modelo cuantitativo NBA especializado
  const quantResult = await runNBAQuantModel({
    researchReport: nbaResearch.report,
    oddsPayload: oddsPayload
  });
  
  // Convertir al formato AnalysisResult
  const analysisResult = convertNBAQuantToAnalysisResult(
    quantResult,
    oddsPayload,
    nbaResearch.report
  );
  ...
  // ═══════════════════════════════════════════════════════════════
  // STEP 3: GROK FINAL VALIDATION
  // ═══════════════════════════════════════════════════════════════

  emitProgress(3, 'running');
  
  const result = await runFinalValidation(analysisResult);
