...
export async function saveAnalysis(analysisData) {
  try {
    ...
    const insertData = {
      ...
      market: analysisData.bestMarket || analysisData.best_pick?.market || '',
      
      // Odds y probabilidades
      odds: parseFloat(analysisData.odds || analysisData.best_pick?.odds || 0),
      estimated_prob: parseFloat(analysisData.estimated_prob || analysisData.best_pick?.probability || 0),
      implied_prob: parseFloat(analysisData.implied_prob || (analysisData.odds ? 1/analysisData.odds : 0)),
      ...
      // Contexto adicional
      user_context: analysisData.userContext || analysisData.user_context || '',
